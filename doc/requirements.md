...
- **R3.5** A column holds one thing. A register's value comes from one
  ...
  prescaler and count.
- **R3.6** A column's top bit says whether the row sets it, and the bits
  under a 0 hold nothing. A row may set a column to the value it already
  held: the bit marks what to take, not what changed.
