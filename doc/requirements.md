...
**ST4's limits.** These bind any format packed by ST4, whichever decoder
reads it.
...
**The decoder is a choice, and it carries its own terms.** ST4 ships two for
a ring destination.

| | |
|---|---|
| `ST4_wrap` | the caller minds the ring: `N` is a whole number of units, `C` divides `N`, and the caller counts its calls and stops at `O`, there being no done state |
| `ST4_ring` | the decoder minds the ring: it stops at the ring end and wraps on the next entry, so a caller asks for any budget and counts nothing |
...
| | |
|---|---|
| the decoder | `ST4_wrap`, and with it `C` dividing `N` and the call counting |
...
| R4.5 | `N` is at least `2C`, and `C` covers the longest list of streams a consumer reads |
