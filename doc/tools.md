...
| `-clock` | the stub's Timer C handler under Hatari's profiler: `Circus Attractions 2` in a program at 50 Hz and at 60 Hz, a row every third tick and every fourth, 1,000 VBLs each; the two runs give the cycles of a tick without a row and of one with a row, and the routine around the play call runs once a row, and performance.md's section on the program's clock reads each within a cycle |
