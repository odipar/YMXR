...
| bitmap | The row buffer's first field: one bit a column, set where that column differs from the previous row. | requirements.md R1.4 |
...
| `C` | The table's column count. At most 32, so the bitmap is one long. | requirements.md R1.1, R3.5 |
...
- **SID voice**, **sync buzzer**, **waveform** - effects R3.1 has the schema
  cover.
