...
|---|---|---|
| bound tune | A tune file's tables bound with DTX's reader into what the player takes: the tune file's layout under its own magic and version, the image where the file has its DTX2 table, and the state block's bytes stated before the index. | BINARIES.md |
...
| image | What DTX packages a table with: its reader's code, its column table and the table, read through four calls at its first bytes. A bound tune holds one; a tune file holds none. | BINARIES.md |
...
| stub | The program stub: a block a tool puts in front of an SNDH file to make a TOS program, which takes the machine over, plays the file from the VBL or Timer C, and hands the machine back. Assembled once and carried in the jar. | BINARIES.md 4 |
...
| tune file | The values a tune states once, its DTX2 table, and one DTX1 table a source, in one file: its tables, and no code. | SPEC.md 3.3 |
...
| workspace | What a host gives the player: the player's own fields, then the state block of the bound tune's image, on a long. | tools.md, the player |
