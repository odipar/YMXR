...
| effect | A source connected to a target on one timer, at the rate its prescaler and count come to. Four columns encode it here: the target, the source, the timer's control and its count. The scene's names: a two-row source on a volume register is a SID voice, a long one is a digidrum, a one-row source on R13 is a sync buzzer. | YMXS, SPEC.md 3; SPEC.md 1.8, 1.9 |
...
| place | The row a timer's next tick reads from its source. A start puts it at the first row, and bit 5 of the control column is the `placeReset` that puts it there again. | YMXS, SPEC.md 3.4; SPEC.md 1.9 |
...
| sample | A recording played through a voice's volume register, one byte a tick. Its rate is the recording's, and it is a source of many rows. | terminology.md; YMXS, SPEC.md 3.2 |
...
| source | A table a tick advances a row at a time, its target writing each row. Here it is a DTX1 table of one column of one byte, repeating at `RR`, its last row the marker, and an effect's source column names one, 1 to 127. | YMXS, SPEC.md 3.2; SPEC.md 2.2, 3.1 |
...
| target | The procedure a timer's tick calls with a source's row. Those named write one register from a one-byte row: `setR0` to `setR13`. An effect's target column numbers one, 0 to 127. | YMXS, SPEC.md 3.1; SPEC.md 2.1 |
...
| wave | One cycle of a signal, played through a register at a timer's rate. A source of a cycle's rows, which its entry usually repeats. | YMXS, SPEC.md 3.2; SPEC.md 2.2 |
...
| YMXS | The tune data structure every conversion here passes through: rows of registers and effects, the sources those effects run, and one rate a tune, encoded as JSON. Its SPEC.md defines the structure and SPEC.md here one encoding of it. A separate repository. | ymxs.md |
